...
def create_booking(
    ...
    # Bug fix: end must be strictly after start
    if end <= start:
    ...
    # Bug fix: check both min and max duration
    ...
    # Bug fix: wrap conflict check + quota check + insert in a lock for SQLite concurrency
    with write_lock:
...
def list_bookings(
    ...
    total = base.count()
    items = (
        # Bug fix: ascending (was desc), offset (page-1)*limit (was page*limit), use limit param (was hardcoded 10)
...
def get_booking(
    ...
    # Bug fix: member visibility — members can only see their own bookings
    if user.role != "admin" and booking.user_id != user.id:
        raise AppError(404, "BOOKING_NOT_FOUND", "Booking not found")

    response = serialize_booking(booking)
    # Bug fix: removed line that overwrote start_time with created_at
    response["refunds"] = [
        {
...
def cancel_booking(
    ...
    # Bug fix: wrap cancel in write_lock to prevent double-cancel race
    with write_lock:
        booking = (
            db.query(Booking)
            .join(Room, Booking.room_id == Room.id)
        ...
        # Bug fix: >=48h (was >48), and <24h gives 0% (was 50%)
    ...
    # Bug fix: invalidate both report AND availability cache on cancel
    cache.invalidate_report(user.org_id)
